...
## ⏩ Session 6 — Learn-tab UX + hands-cause-the-motion pass (wash / hindu / strip / charlier)

User-directed fixes, all verified with puppeteer screenshots (driver pattern below) + `npm run verify` + lint + build clean.
...
2. **Removed lessons:** spring primer + pile shuffle (files deleted, catalog index updated). Catalog is 9 techniques.
...
**Remaining known nits:** swinging charlier packet still grazes the top half's edge mid-pivot; wash palms visually brush at closest approach; hindu/strip right-hand carry shows the deck slightly below the fingertip cage (pinch-frame offset). Next: the harder flourishes (springs/waterfall polish, faro precision).
