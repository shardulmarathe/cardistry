...
## ⏩ Session 5 update — FINGER-DRIVEN overhaul (contact frames, per-card releases, riffle + charlier flagships)

Plan: `~/.claude/plans/the-highest-priority-for-glowing-iverson.md`. Priority shifted: cards must visibly be MOVED BY the fingers (wrist only repositions). Everything verified headlessly by a **checked-in** harness: `npm run verify` (fkParity 3340 checks + verifyTracks ~440k checks: purity/scrub-reversibility, NaN, boundary continuity, riffle + charlier grip-fidelity). Lint + build clean. **Needs the user's eyes** — all geometry still authored blind.

**Architecture added (all deterministic, pure fn of ms):**
1. **`src/hands/handRigSpec.js`** — single source of truth for rig numbers (FINGERS table, HAND_SCALE, THUMB_BASE_ROT, JOINT_LIMITS). `handRig.js` imports it.
2. **🐛 Joint-pivot fix** (`handRig.js` buildFinger): joints previously ALL pivoted at the knuckle (fingers fanned, never curled). Now each joint pivots at its phalange's base. Changes every pose's silhouette (tips moved up to 0.36 world units on grippy poses).
3. **`src/hands/handKinematics.js`** — pure FK (fingertip/joint world positions from a pose, headless-safe, mirror-safe: points may mirror, orientations only ever composed `wrist.quat ∘ f(angles)`); analytic 2-link finger IK (`solveFingerTo`, distal coupling 0.75) + thumb IK with opposition search (`solveThumbTo`); **contact frames** `GRIP_FRAME_TYPES`: `pinch` (thumb+index mid), `packet` (thumb/index/middle centroid), `thumbPeel` (thumb-dominant), `indexPivot` (index tip, pitchGain −2.2 → charlier swing); `applyGripPressure` (pressure visibly tightens gripping fingers). `fkParity.test.mjs` guards FK ↔ rig < 1e-6 forever.
4. **Pose schema v2** (back-compat, defaults=zeros): optional `splay:{perFinger}` + `thumbOpp:{x,z}` (animatable opposition); keyframes may carry partial `splay`/`thumbOpp` overrides and **pose OBJECTS** (not just names).
5. **Grip spec v2** (`compileLesson.js`): `grip:{side:{cards, frame, pressure:[{at,v}], bendGain, release}}`. Legacy string form still = rigid wrist weld. `frame` puts held cards on a fingertip contact frame → **finger curls move the cards**. `pressure` tightens the hand + `bendGain` bows the held packet. `release:'stagger'` = per-card release at its own stagger segment start.
6. **Snap-killer release baking** (`bakeHoldReleases`): every hold projects each card through `frame(t_release)∘offset` and overwrites the card's next segment `from` → handoffs are seamless BY CONSTRUCTION (riffle max boundary jump 0.0019; was 0.03). Hold captures CHAIN through prior holds (`renderedCardPose`) so grips hand packets to each other (arch → weave).
7. **Micro-animation** (`src/hands/handMotion.js`): global idle breathing (pure fn of absolute ms, per-finger phases, side-decorrelated — hands NEVER freeze, all 11 lessons, incl. grip-capture path so welds stay exact); per-keyframe `fingerMotion:[{fingers,type:'tremor'|'curlRipple'|'tighten',amp,cycles}]` (sin²(πt) envelope → boundary-safe). New eases: `easeOutBackSoft`, `anticipate`, `settle`, `snapEase` (`src/lib/ease.js`).
8. **Compile-time contact authoring** (`src/lessons/authoring/contacts.js`): `poseWithContacts` (IK fingertips onto card-edge targets inside `build()` — no more blind angle guessing), `wristAnchorForContact`, `thumbRatchetKeyframes` (thumb opens across exactly the stagger release window), `eulerQuat`. Compiler fix: partial `to` arrays now carry unlisted cards' poses forward.

**Riffle re-authored** (`riffle.lesson.js`, ~22.6s, stage grammar approach→grip→separation→pressure→transfer→alignment→closure→recovery): hands reach in FIRST; right hand cuts the top half off the stack and carries it on edge (packet rides the fingertip frame — rotation/translation all hand-caused); left tips its half; both clamp with a **build-time-solved side grip** (palm toward the packet's outer face, fingers stacked down it, thumb ON the top release edge — all five tips land within 0.06 of their targets); pressure→1 loads the bend (+`bendGain` squeeze) with thumb tremor; **weave**: both halves pinched in `thumbPeel` frames, thumbs ratchet open (`thumbRatchetKeyframes`, alternating micro-jitter) and **each card leaves at its own release moment from wherever the thumb holds it**; settle-eased square; bridge cages the deck (rides right hand's `packet` frame); cascade = per-card releases from the cage with a `curlRipple` on the fingers; recovery to `relaxed` (idle carries on).

**Charlier re-authored** (`charlier.lesson.js`, ~9.5s, one hand): palm-up pinch pickup (solved contacts: fingers across the far long edge, thumb the near); **thumb visibly releases** the bottom half into the palm (wrist provably still — harness asserts <0.09 drift through the finger beats); **index extends and the packet swings up-and-over riding the index fingertip's frame** (`indexPivot`), released mid-air at the apex (baked handoff), gravity lands it on the top half; thumb catch (`tighten`); hover + set-down recovery (final hand keyframe is the pinch pose translated by exactly the deck's travel so the weld releases with zero error — the harness caught a 0.38 snap in the naive version).

**Tuning knobs for the visual pass:** clamp grip targets/anchor → `clampContacts`/`CLAMP_ANCHOR` in `riffle.lesson.js`; charlier deck height/pinch → `DX/DY/DZ`, `PINCH_ANCHOR`, contact targets in `charlier.lesson.js`; swing size/direction → `indexPivot.pitchGain` (`handKinematics.js`); idle amount → `IDLE_*` consts in `handMotion.js`; pressure curl depth → `PRESSURE_CURL` (`handKinematics.js`); frame pitch coupling → `pitchGain` per frame type.

**Deferred (explicit):** fingertip contact-dent shader (needs `uContact[4]` uniforms + `X_SEGS` 6→16 in `cardGeometry.js` — pressure-bend + tighten carries the read at current camera distances); square-step cards still keyframed (hands nudge alongside); possible GLB swap stays possible (FK/pose interface is rig-agnostic).

### Session 5b — TABLE riffle + finger-driven pass across the WHOLE catalog (user direction: keep cards flat/low, real casino mechanics)

Verified **visually via puppeteer screenshots** this time (dev server + `scratchpad/drive.mjs` driver: navigates Learn → lesson, scrubs the range input, screenshots — the repo's puppeteer devDependency drives it; consider `/run-skill-generator` to make this a project skill). Screenshots confirmed: table-riffle bend/weave read like a real dealer; earlier on-edge design scrapped per user feedback.

- **`tableRiffleLayout`** (`layouts.js`): two face-down halves FLAT on the felt, side by side, inner corners angled (`yaw`), `tilt` lifts each half's near edge as the thumbs load it (position compensates so the far edge stays down).
- **Shared solved grips** (`authoring/contacts.js`): `tableGrip({gap, tilt})` — palm flat over a half, fingertips ON the cards, thumb owning the inner-near corner (solve validated <0.05 err); `cageGrip({topY})` — cups the center deck's short end, thumb on top. Used by riffle, faro, waterfall, spring-primer.
- **Riffle** re-authored as the real TABLE riffle (~19.6s): reach in → cut top half flat beside → slide other half → thumbs tilt+bend corners (pressure/bendGain) → thumb-ratchet weave, cards flick down LOW (`midBend 0.7, arcLift 0.05`) → settle square → **cage-then-bow bridge** (hands take cage position FIRST, then grip+pressure bows the deck IN PLACE — gripping while the hand rotates 90° tips the deck over; screenshot-caught) → per-card cascade → rest. Annotations moved to `[-1.35, 0.75, 0.2]` (beside the action, never covering it) — do this in all lessons.
- **Faro**: precision variant of the same table system (tighter yaw 0.16, tilt 0.18, `midBend 0.4/arcLift 0.03`, jitter 0.015, 7-step ratchet).
- **Hindu/Strip**: deck rides the right hand's `pinch`; `release:'stagger'` peels each packet out of the grip at its own draw moment; left hand sweeps/presses. Falls use `snapEase` + packet stagger.
- **Overhand**: carries upgraded to `frame:'packet'` + pressure (squeeze at pickup, relax into drop).
- **Waterfall/Spring-primer**: cage-first, pressure bows (`bendGain`), waterfall = per-card `release:'stagger'` out of the cage with `curlRipple`.
- **Pile/Pressure-fan**: light pass (no grips): dealing orbit + tremor synced to card stagger; fan pivot pinch + sweeping `fanSpread` with `splay` overrides.
- All 11 lessons: `npm run verify` green (520k checks incl. riffle/charlier fidelity), lint/build clean. Watch-outs found via screenshots: never grip a packet across a large hand-orientation change (tips the cards); keep converging hands ≥0.5 apart in x or palms interpenetrate.
