...
## ⏩ Session 6 — Learn-tab overhaul: causal hand choreography, landscape riffle, felt floor

**Committed + pushed: `b0baa16` on master** (23 files, +986/−815). All user-directed fixes, each verified with puppeteer screenshots (driver pattern below) + `npm run verify` (~630k checks) + lint + build clean.

1. **Instructions moved ABOVE the simulation.** The 3D drei-Html annotation callouts covered the cards; deleted `AnnotationLayer.jsx` and added a DOM banner `src/ui/LessonInstructions.jsx` (mounted in `UIChrome`, styled `.lesson-instructions` in `chrome.css`) that shows the active annotation text docked under the header. Lesson `annotations:` keep their timing fields; their `at:` world positions are now unused.
2. **Removed lessons:** spring primer, pile shuffle, and pressure fan (files + catalog entries + dead `pressureFanLayout`). Catalog is 8 techniques.
3. **Charlier no longer impales the deck.** Root cause: the old `lift` step held a `pinch` grip across the ~180° palm-down→palm-up wrist turn, so the rigid grip tipped the deck on-edge through the fingers for the whole cut (the exact session-5 watch-out). Now `lift` has NO grip — the deck rises flat on its own track while the hand rotates around it — and a new `settle-grip` hold grips only after the hand is in its final orientation. Also raised the pinch finger contacts to the deck's TOP far corner (they were solved at its base, so fingers crossed through the 0.21-thick stack) and the thumb to the top half's base so the bottom half drops out beneath it.
4. **Hindu rebuilt as real carries** (`hindu.lesson.js`): right hand picks the deck up (grip while palm-down, orientation never changes) and shuttles between a hover home and the left palm; a new `palmCradle` pose (palm-up, in `handPoses.js`) receives; each strip = `carry-k` (hand+deck to the palm) then `strip-k` (grip re-declared WITHOUT the block → the packet detaches exactly where the hand held it, snapEase-falls ~0.15 onto the palm pile, cradle fingers `tighten`). The left hand then carries the finished pile to center (grip left, `packet` frame; anchor chosen so the weld lands the pile on the center stack), both hands square at ±0.42. Block order = packets taken off the TOP, first-stripped at pile bottom (physical block reversal, no arbitrary shuffleArray).
5. **Strip rebuilt with both hands working** (`strip.lesson.js`): same carry/strip skeleton but drops land on a TABLE pile at center; the left palm waits clear at world −0.6 and PRESSES each landed packet flat (washFlat descent timed AFTER the right hand retreats — they never share the pile's airspace), then both hands push the pile together from ±0.42.
6. **Wash: cards only move when a palm drags them** (`wash.lesson.js`, rewritten): each palm owns one half — counter-rotating mirrored orbits around C=(±0.48, 0.25), AMP 0.28 (**circles must not overlap: both palms reach their inner point simultaneously**, 0.38 blobbed at table center). Every card is assigned to the nearer orbit, swirled about THAT center with an `exp(−((r−AMP)/0.5)²)` falloff on distance from the palm's actual path, and its stagger window is sorted by `passTime` = when that palm sweeps through its angle. Spread = palms press the stack then spiral out (cards leave nearest-first, interleaved by side). Gather = two PLOW sweeps: right palm sweeps in from x=1.45 pushing its half to the stack (nearest-the-plow first), left palm waits as the wall, then roles swap, then square. Scatter radius 1.0 keeps everything within palm reach.
7. **Transport nit fixed:** `player.scrubTo` now recomputes `stepIndex`, so the "Step x/y" label tracks paused scrubbing.
8. **Riffle/faro staging turned LANDSCAPE** (user: halves must be turned 90°, split left/right, hands rotated to match, nothing under the felt). `tableRiffleLayout` rebuilt: halves lie long-axis-left-right at ±gap, base yaw `s·(π/2 − yaw)` (yaw = small inward angle), **always face-down** (`faceQuat(false)` — the old layout honored `card.isFaceUp`, which is why a face-up deck showed card faces mid-riffle), and `tilt` now lifts each half's INNER end pivoting exactly on the outer end (`y += (H/2)sin(tilt)`, x pulled toward the pivot) so **no corner can dip below the table** (the old near-edge tilt clipped through the felt). New `landscapeStackLayout` = squared 90°-yawed stack; the `riffle` step kind takes an optional `toLayout:` (compileLesson) so the weave lands landscape, and square/bridge/cascade stay landscape (short ends at ±0.44 face the hands — natural bridge orientation, bend arches left-right). `tableGrip` re-solved: hand quat `Euler(π/2, −π/2+0.12, 0, 'YXZ')` = palm down, **fingers pointing inward toward the weave, forearm trailing off the outer end** (the 90° hand rotation; left hand is the free x-mirror), thumb at the inner-near corner, contact heights follow the tilted card plane; wrist rolls up with the tilt (`rotZ(−tilt·0.85)` premultiplied). `cageGrip` retargeted to the landscape ends (x≈0.46, anchor 0.72). Harness note: the riffle-grip fidelity threshold is 0.7 (tip → nearest card CENTER; landscape cards are 0.88 long so cupping an end face is ~0.45 from the nearest center). Faro shares all of it (yaw 0.07, tilt 0.14). Screenshot-verified: staging, load-tilt, weave, bridge, cascade.

9. **Cards can no longer go inside the table — two-layer fix.** (a) Root cause of the user's screenshots: the visualizer had left cards FACE-UP in the deck state; the riffle's face-down targets then made each such card somersault 180° mid-weave — standing on edge, face out, half a card-length under the felt. `compileLesson` now normalizes the incoming deck face-down (free: entering a lesson already teleports cards into the start stack). (b) Engine guarantee: `sampleTrack.clampAboveFelt` — for every rendered card, compute the lowest corner from the quat's rotated width/length axes and push the card up so it never crosses y=0.012. Pure + continuous, so scrub purity and boundary budgets hold. Harness now asserts the felt floor on EVERY sampled frame of every lesson plus a dedicated all-face-up riffle regression (~630k checks).

**Puppeteer driver** (scratchpad `shot.mjs`, works headless against `npm run dev`): goto :5173 → click "Learn" → click lesson button by text → set `.scrub input[type=range]` via the native value setter + `input` event → wait ~700ms → screenshot. Re-create it next session; it caught every real bug this session (charlier on-edge deck, wash center-blob, strip hand overlap).

**Remaining known nits:** swinging charlier packet still grazes the top half's edge mid-pivot; wash palms visually brush at closest approach; hindu/strip right-hand carry shows the deck slightly below the fingertip cage (pinch-frame offset). Per the user, next up: **the harder shuffling techniques** — overhand is the last lesson with only wrist-weld carries, and waterfall/spring polish is still open. New invariants to respect when authoring: cards must never cross y=0.012 (clampAboveFelt asserts it everywhere), and lessons always start from a face-down deck (compileLesson normalizes).
